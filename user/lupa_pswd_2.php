...
$sql = mysql_query("SELECT * FROM user WHERE id_user='".$_SESSION["ses_lupa_1"]."'AND ayah_user='".$_POST['ayah_user']."'AND ibu_user='".$_POST['ibu_user']."'") or die (mysql_error());
$cek=mysql_num_rows($sql);
$data_u=mysql_fetch_array($sql);

	if ($cek >=1 ){
		$_SESSION["ses_lupa_2"]=$data_u["id_user"];
		$_SESSION["ses_name_2"]=$data_u["nama_user"];
		
		echo "<meta http-equiv='refresh'content='0; url=lupa_pswd_reset.php'>";
		
		
		$sql_insert_log = "Insert into log(id_user,isi_log) values(
			'".$_SESSION['ses_lupa_2']."',
			'Mengggunakan fasilitas Lupa Password pada Tahap Akhir')";
		$query_insert_log = mysql_query($sql_insert_log) or die (mysql_error());
	
	} else {
		echo "<div class='alert alert-danger'>
			<a href='' class='close' data-dismiss='alert'>&times;</a>Maaf !...   Nama orang tua anda tidak cocok...		</div>";
	}
}
              ...
              <h1>Lengkapi Formulir</h1>
					  <div class="item form-group">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                          <input id="name" class="form-control col-md-7 col-xs-12" name="nama_user" value="<?php echo $_SESSION["ses_name_1"]; ?>" type="text" readonly="readonly">
